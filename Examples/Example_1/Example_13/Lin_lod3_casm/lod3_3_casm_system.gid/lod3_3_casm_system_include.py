...
#                        #item.Free(AIR_PRESSURE)
    ...
    def InitializeModel( self ):
        ...
        self.model_part.Properties[2].SetValue(DENSITY,          300 )        
        ...
        print "Linear elastic model selected for Bolts, description: Isotropic3D"

        self.model_part.Properties[4].SetValue(DENSITY,         2500 )        
        self.model_part.Properties[4].SetValue(YOUNG_MODULUS,        2e+10 )        
        ...
        self.model_part.Properties[5].SetValue(DENSITY,         7620 )        
        self.model_part.Properties[5].SetValue(YOUNG_MODULUS,      2.1e+11 )        
        ...
        params1 = Vector(19)
        params1[0] =     5.25e+06 #Young's Modulus
        params1[1] =         0.35  #Poisson Ratio
        params1[18] = 0
        if( linear_elastic == False ):
            print "--> Clay and Sand Model"
            print "!      IMOD = 1 . SATURATED SOIL"

            print "!         1.- Poisson's ratio ------------------- (    poiss)"
            print "!         2.- Slope of the unload/reload curve--- (   xkappa)"
            print "!         3.- Slope of the normal comp curve----- (  xlambda)"
            print "!         4.- Spacing ratio --------------------- (        r)"
            print "!         5.- Shape parameter of the yiel funct-- (       xn)"
            print "!         6.- Slope of the CSL------------------- (       xM)"
            print "!         7.- Friction angle at CS (degrees) -----(      phi)"
            print "!         8.- Initial void ratio ---------------- (       e0)"
            print "!         9.- Initial Preconsolidation pressure-- (       Po)"
            print "!        10.- Undrained shear strength ---------- (       Cu)"



            #data from janosch/simulations_jelena
            params1[2] = 0.0006
            params1[3] = 0.011
            params1[4] = 2.0
            params1[5] = 2.0
            ...
            self.model_part.Properties[3].SetValue(MATERIAL_PARAMETERS, params1 )
            self.model_part.Properties[3].SetValue(CONSTITUTIVE_LAW, UPCSoil_explicit())
            self.model_part.Properties[6].SetValue(MATERIAL_PARAMETERS, params1 )
            self.model_part.Properties[6].SetValue(CONSTITUTIVE_LAW, UPCSoil_explicit())
        else:
            self.model_part.Properties[3].SetValue(MATERIAL_PARAMETERS, params1 )
            self.model_part.Properties[3].SetValue(CONSTITUTIVE_LAW, UPCSoil())
            self.model_part.Properties[6].SetValue(MATERIAL_PARAMETERS, params1 )
            self.model_part.Properties[6].SetValue(CONSTITUTIVE_LAW, UPCSoil())

        self.soil_elems = []
        self.soil_elems.extend(self.layer_sets['ground'])
        for step in range(1, 28):
            self.soil_elems.extend(self.layer_sets['excavation_' + str(step)])
        for e in self.soil_elems:
            elem = self.model_part.Elements[e]
            elem.SetValue(DENSITY, 1700.0)
